...
export default function BlendedFontHebrewSelection(
  ...
  const WebFontsSelectableHebrew = webFontsHebrew.map((font, index) => (
  ...
  const neutralScope = " ";
  const regexHebrew = RegExp(
    `[^(${unicodeRangeHebrew}||${neutralScope})]`,
    "ugm",
  );
  ...
  const exampleHebrewDir = useDetectDir({
    text: exampleHebrew,
    isMarkup: false,
    ratioThreshold: 0.51,
  });
