...
export default function FontFeaturesGreek(fontFeaturesGreekProps) {
  const {
    greekFontSettings, // Selected options (initially default)
    ...
    ffsArr, // Options
    exampleRegex,
    setExampleGreek, // Example text
  } = fontFeaturesGreekProps;
  ...
  useEffect(() => {
    if (fontSettingsArr.length !== 0) {
      setGreekFontSettings(fontSettingsArr);
    }
  ...
  useEffect(() => {
    if (fontSettingsArr.length !== 0) {
      ...
      const exampleStr = 'Ἐν ἀρχῇ ἦν ὁ λόγος, καὶ ὁ λόγος ἦν πρὸς τὸν θεόν, καὶ θεὸς ἦν ὁ λόγος. ' + labelStr.replace(exampleRegex, '');
      setExampleGreek(exampleStr);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[ffsId]);

  const handleChange = useMemo(() => (event) => {
    const newState = greekFontSettings.map(obj => {
      ...
      setGreekFontSettings(newState);
  },[greekFontSettings, setGreekFontSettings]);
  ...
  const fontFeatureSettingsProps = {
    fontDisplayName,
    fontSettings: greekFontSettings,
    handleChange,
    placementDir,
    radioRightMargin,
    radioLeftMargin,
...
FontFeaturesGreek.propTypes = {
  /** Greek Font Settings (initially default) */
  greekFontSettings: PropTypes.array,
  /** Set Greek Font Settings */
  setGreekFontSettings: PropTypes.func.isRequired,
  ...
  /** Set Example Greek Text */
  setExampleGreek: PropTypes.func.isRequired,
};