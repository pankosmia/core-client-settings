...
export default function FontFeatureDefaults(FontFeatureDefaultsProps) {
  const { ffsArr, isGraphite } = FontFeatureDefaultsProps;

  // This creates an array of font settings names and default values
  const fontSettingsJsx = useMemo(
    () =>
      ffsArr.map((font, fontIndex) => (
        <div key={fontIndex}>
          {font.categories.map((categories, categoriesIndex) => {
            return (
              <div key={categoriesIndex}>
                {font.categories[categoriesIndex].category
                  .filter((item) =>
                    isGraphite
                      ? item.opentype_render_required !== true
                      : item.graphite_render_required !== true,
                  )
  ...
  const fontSettingsStr = renderToString(fontSettingsJsx).replace(
    /(<([^>]+)>)/gi,
    "",
  );
  // remove the last comma, change [] to {} and ~ to " and convert string to an array of objects
  const fontSettingsAdj =
    "[" +
    fontSettingsStr
      .substring(0, fontSettingsStr.length - 1)
      .replace(/\[/gm, "{")
      .replace(/\]/gm, "}")
      .replace(/~/gm, '"') +
    "]";
