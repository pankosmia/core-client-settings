...
export default function BlendedFontArabicUrduSelection(
  ...
  // Example text
  const [exampleArabicUrdu, setExampleArabicUrdu] = useState(
  ...
  useEffect(() => {
    ...
    const selectedArabicUrduSettingsId = webFontsArabicUrdu
      .filter((font) => font.id === selectedArabicUrduFontClassSubstr)
      ?.map((font, index) => font.settings_id);
    setArabicUrduFfsId(selectedArabicUrduSettingsId);
    const selectedArabicUrduSettingsArr = ffsArr.filter((item) =>
      selectedArabicUrduSettingsId.includes(item.name),
    );
  ...
  const handleChangeArabicUrdu = (event) => {
    ...
    setArabicUrduFfsCss("");
    const selectedArabicUrduSettingId = webFontsArabicUrdu
      .filter((font) => font.id === event.target.value)
      .map((font, index) => font.settings_id);
    setArabicUrduFfsId(selectedArabicUrduSettingId);
    const selectedArabicUrduSettingsId = ffsArr.filter((item) =>
      selectedArabicUrduSettingId.includes(item.name),
    );
  ...
  const WebFontsSelectableArabicUrdu = webFontsArabicUrdu.map((font, index) => (
    <MenuItem key={index} value={font.id} dense>
      <FontMenuItem font={font} {...fontMenuItemProps} />
    </MenuItem>
  ));

  const ffsArabicUrduFontName = arabicUrduFontName
    .toString()
    .replace("Pankosmia-", "")
    .replace(/ /g, "_");
  ...
  useEffect(() => {
    if (arabicUrduFontSettings.length !== 0) {
      ...
      const arabicUrduFfsStr = renderToString(arabicUrduFfsJsx)
        .replace(/(<([^>]+)>)/gi, "")
        .replace(/~/gm, '"');
      // remove the last comma, change ~ to "
      const arabicUrduFfsCssStrNext = arabicUrduFfsStr
        .substring(0, arabicUrduFfsStr.length - 1)
        .replace(/~/gm, '"');
      ...
      if (arabicUrduFfsCss !== "") {
        ...
        for (let i = 0; i < prevArabicUrduFfsCssArr.length; i++) {
          if (nextArabicUrduFfsCssArr[i] !== prevArabicUrduFfsCssArr[i]) {
            const ffsStr = renderToString(nextArabicUrduFfsCssArr[i])
              .replace(" &quot;", "")
              .replace("&quot; ", "/");
            postEmptyJson(
  ...
  const unicodeRangeArabicUrdu =
    selectedArabicUrduFontClassSubstr === ""
      ? ""
      : unicodeRanges
          .filter((item) => item.name === "Arabic/Urdu")
          .map((script, index) => script.unicode_range);
  const neutralScope = " ";
  const regexArabicUrdu = RegExp(
    `[^(${unicodeRangeArabicUrdu}||${neutralScope})]`,
    "ugm",
  );
  ...
  const exampleArabicUrduDir = useDetectDir({
    text: exampleArabicUrdu,
    isMarkup: false,
    ratioThreshold: 0.51,
  });
        ...
        <div className={adjSelectedFontClass} style={{ fontSize: "100%" }}>
          <Stack direction="row">
            ...
            {isAwami && !isGraphite && (
              ...
              >
                {doI18n("pages:core-settings:best_with", i18nRef.current)}{" "}
                {isFirefox &&
                  doI18n(
                    "pages:core-settings:graphite_is_off",
                    i18nRef.current,
            ...
            {showArabicUrduFeatures && (
              <FontFeaturesArabicUrdu {...fontFeaturesArabicUrduProps} />
            )}
          </Stack>
        </div>
      </Grid2>
          ...
          {showArabicUrduTextArea && (
            <TextareaAutosize
              ...
              style={{
                ...
                fontFeatureSettings:
                  arabicUrduFfsCss !== "" && arabicUrduFfsCss,
