...
export default function BlendedFontMyanmarSelection(
  ...
  // Example text
  const [exampleMyanmar, setExampleMyanmar] = useState(
    "အစဦး၌ နှုတ်ကပတ်တော်ရှိ၏။ နှုတ်ကပတ်တော်သည် ဘုရားသခင်နှင့်အတူရှိ၏။ နှုတ်ကပတ်တော် သည်လည်း ဘုရားသခင်ဖြစ်တော်မူ၏။",
  );

  /** Set INITIAL options array (FfsArr) */
  useEffect(() => {
    const selectedMyanmarSettingsArr = ffsArr.filter((item) =>
      myanmarFfsId.includes(item.name),
    );
  ...
  const handleChangeMyanmar = (event) => {
    ...
    setMyanmarFfsCss("");
    const selectedMyanmarSettingId = webFontsMyanmar
      .filter((font) => font.id === event.target.value)
      .map((font, index) => font.settings_id);
    setMyanmarFfsId(selectedMyanmarSettingId);
    const selectedMyanmarSettingsId = ffsArr.filter((item) =>
      selectedMyanmarSettingId.includes(item.name),
    );
  ...
  const WebFontsSelectableMyanmar = webFontsMyanmar.map((font, index) => (
    <MenuItem key={index} value={font.id} dense>
      <FontMenuItem font={font} {...fontMenuItemProps} />
    </MenuItem>
  ));

  const ffsMyanmarFontName = myanmarFontName
    .toString()
    .replace("Pankosmia-", "")
    .replace(/ /g, "_");
  ...
  useEffect(() => {
    if (myanmarFontSettings.length !== 0) {
      ...
      const myanmarFfsStr = renderToString(myanmarFfsJsx)
        .replace(/(<([^>]+)>)/gi, "")
        .replace(/~/gm, '"');
      // remove the last comma, change ~ to "
      const myanmarFfsCssStrNext = myanmarFfsStr
        .substring(0, myanmarFfsStr.length - 1)
        .replace(/~/gm, '"');
      ...
      if (myanmarFfsCss !== "") {
        ...
        for (let i = 0; i < prevMyanmarFfsCssArr.length; i++) {
          if (nextMyanmarFfsCssArr[i] !== prevMyanmarFfsCssArr[i]) {
            const ffsStr = renderToString(nextMyanmarFfsCssArr[i])
              .replace(" &quot;", "")
              .replace("&quot; ", "/");
            postEmptyJson(
  ...
  const neutralScope = " ";
  const regexMyanmar = RegExp(
    `[^(${unicodeRangeMyanmar}||${neutralScope})]`,
    "ugm",
  );
  ...
  const exampleMyanmarDir = useDetectDir({
    text: exampleMyanmar,
    isMarkup: false,
    ratioThreshold: 0.51,
  });
