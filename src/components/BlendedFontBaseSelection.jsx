...
export default function BlendedFontBaseSelection(
  ...
  // Example text
  const [exampleBase, setExampleBase] = useState(
    "Au commencement... / In the beginning...",
  );

  /** Set INITIAL options array (FfsArr) */
  useEffect(() => {
    const selectedBaseSettingsArr = ffsArr.filter((item) =>
      baseFfsId.includes(item.name),
    );
  ...
  const handleChangeBase = (event) => {
    ...
    setBaseFontSettings([]);
    setBaseFfsCss("");
    const selectedBaseSettingId = webFontsBase
      .filter((font) => font.id === event.target.value)
      .map((font, index) => font.settings_id);
    setBaseFfsId(selectedBaseSettingId);
    const selectedBaseSettingsId = ffsArr.filter((item) =>
      selectedBaseSettingId.includes(item.name),
    );
  ...
  const WebFontsSelectableBase = webFontsBase.map((font, index) => (
    <MenuItem key={index} value={font.id} dense>
      <FontMenuItem font={font} {...fontMenuItemProps} />
    </MenuItem>
  ));

  const ffsBaseFontName = baseFontName
    .toString()
    .replace("Pankosmia-", "")
    .replace(/ /g, "_");
  ...
  useEffect(() => {
    if (baseFontSettings.length !== 0) {
      ...
      const baseFfsStr = renderToString(baseFfsJsx)
        .replace(/(<([^>]+)>)/gi, "")
        .replace(/~/gm, '"');
      // remove the last comma, change ~ to "
      const baseFfsCssStrNext = baseFfsStr
        .substring(0, baseFfsStr.length - 1)
        .replace(/~/gm, '"');
      ...
      if (baseFfsCss !== "") {
        ...
        for (let i = 0; i < prevBaseFfsCssArr.length; i++) {
          if (nextBaseFfsCssArr[i] !== prevBaseFfsCssArr[i]) {
            const ffsStr = renderToString(nextBaseFfsCssArr[i])
              .replace(" &quot;", "")
              .replace("&quot; ", "/");
            postEmptyJson(
  ...
  const unicodeRangeHebrew =
    selectedHebrewFontClassSubstr === ""
      ? ""
      : unicodeRanges
          .filter((item) => item.name === "Hebrew")
  ...
  const unicodeRangeGreek =
    selectedGreekFontClassSubstr === ""
      ...
      : unicodeRanges
          ...
          .map((script, index) => script.unicode_range);
  const unicodeRangeMyanmar =
    selectedMyanmarFontClassSubstr === ""
      ? ""
      : unicodeRanges
          .filter((item) => item.name === "Myanmar")
          .map((script, index) => script.unicode_range);
  ...
  const regexBase = RegExp(
    `[${unicodeRangeHebrew}${unicodeRangeGreek}${unicodeRangeMyanmar}${unicodeRangeArabicUrdu}]`,
    "ugm",
  );
  ...
  const exampleBaseDir = useDetectDir({
    text: exampleBase,
    isMarkup: false,
    ratioThreshold: 0.51,
  });
  ...
  const baseExampleFontName = baseFontName
    .toString()
    .includes("Pankosmia-Gentium")
    ? baseFontName
      ...
      <Grid2 size={12}>
        <div className={adjSelectedFontClass} style={{ fontSize: "100%" }}>
          <Stack direction="row">
            ...
            {showBaseFeatures && (
              <FontFeaturesBase {...fontFeaturesBaseProps} />
            )}
          </Stack>
        </div>
        <Grid2 size={12}>
