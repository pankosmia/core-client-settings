...
export default function FontFeaturesOther(fontFeaturesOtherProps) {
  const {
    otherFontSettings, // Selected options (initially default)
    ...
    ffsArr, // Options
    // exampleRegex,
    // setExampleOther, // Example text
  } = fontFeaturesOtherProps;
  ...
  useEffect(() => {
    if (fontSettingsArr.length !== 0) {
      setOtherFontSettings(fontSettingsArr);
    }
  ...
  /** Example text for Other will come from Fallback instead of coming from here.
  useEffect(() => {
    if (fontSettingsArr.length !== 0) {
  ...
  */
  ...
  const handleChange = useMemo(() => (event) => {
    const newState = otherFontSettings.map(obj => {
      ...
      setOtherFontSettings(newState);
  },[otherFontSettings, setOtherFontSettings]);
  ...
  const fontFeatureSettingsProps = {
    fontDisplayName,
    fontSettings: otherFontSettings,
    handleChange,
    placementDir,
    radioRightMargin,
    radioLeftMargin,
...
FontFeaturesOther.propTypes = {
  /** Other Font Settings (initially default) */
  otherFontSettings: PropTypes.array,
  /** Set Other Font Settings */
  setOtherFontSettings: PropTypes.func.isRequired,
  ...
  /** Set Example Other Text */
  setExampleOther: PropTypes.func.isRequired,
};